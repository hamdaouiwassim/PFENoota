            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    ...
                    <div style="display:flex;">
                        <h2 style="width:80%;"> <i class="fa fa-fw fa-files-o text-blue"></i> إحصائياتي   </h2>
                    ...
                    <div class="">
                        ...
                        <div >
                            ...
                            <p>إجمالي عدد التعليقات التي تحصلت عليها هذا الشهر لمنشوراتك <span style="color:blue;">{{ $Monthcomments }} تعليقات</span>  </p>
                            <p> إجمالي عدد التفاعلات على منشوارتك في هذا الشهر <span style="color:blue;">{{ $Monthreactions }}  تفاعلات </span></p>
                        </div>
                        <div class="bestMonthlyPost">
                            <hr>
                            <h5 style="text-align:center;">
                                    أفضل منشور خاص بك لهذا الشهر
                                </h5>
                            <div class="posts rtl">
                                @if (!empty($Monthpost))
                                    @foreach($users as $user )
                                        @if ($user->id == $Monthpost->Iduser)
                                            <div class="post col-md-12 well rtl" style="margin-bottom: 50px;" >
                                ...
                                <a href="/user/{{ $user->id }}" > {{ $user->name }}</a>
                                                    ...
                                                    <br/>
                                                    <br/>
                                                    <span class="description"> نشر {{ \Carbon\Carbon::parse($Monthpost->updated_at)->diffForHumans()  }}</span>
                                                </div>
                                                <div class="post_details col-md-8 rtl">
                                                    <div class="title">
                                                        <h4>{{ $Monthpost->Title }}</h4>
                                                    </div>
                                                    <div class="description">
                                                        <p>{!! $Monthpost->Content  !!}</p>
                                                    </div>
                                                    <div class="details">
                                                        <a href="#" class="btn btn-primary" style="font-size:10px;"><i class="fa fa-comment margin-r-5"></i> {{ $comments[$Monthpost->Id] }} </a>
                                                        <a href="#" class="btn btn-default" style="font-size:10px;"><i class="fa fa-heart margin-r-5" style="color:red;"></i> {{ $reactions[$Monthpost->Id] }} </a>

                                                        <a href="/post/{{ $Monthpost->Id }}" class="btn btn-success" style="font-size:10px;">شاهد المقال </a>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                        ...
                        <hr>
                        <h3>إحصائيات منذ تسجيل العضوية  </h3>
                        <hr>

                        <div >

                            <p>إجمالي عدد التعليقات التي تحصلت عليها  لمنشوراتك <span style="color:blue;">{{ $AllTimecomments }} تعليقات</span> </p>
                            <p> إجمالي عدد التفاعلات على منشوارتك  <span style="color:blue;">{{ $AllTimereactions }}  تفاعلات </span></p>
                        </div>
                        <div class="bestMonthlyPost">
                            <hr>
                            <h5 style="text-align:center;">
                            ...
                            <div class="posts rtl">
                                @if (!empty($AllTimepost))
                                @foreach($users as $user )
                                    @if ($user->id == $AllTimepost->Iduser)
                                        <div class="post col-md-12 well rtl" style="margin-bottom: 50px;" >
                                            <div class="user_details col-md-4 rtl " style="text-align:right">
                                                @if ($user->logo != "")
                                                    <img src="images/{{ $user->logo }}" class="userlogo" >
                                ...
                                <a href="/user/{{ $user->id }}" > {{ $user->name }}</a>
                                                ...
                                                <br/>
                                                <br/>
                                                <span class="description"> نشر {{ \Carbon\Carbon::parse($AllTimepost->updated_at)->diffForHumans()  }}</span>
                                            </div>
                                            <div class="post_details col-md-8 rtl">
                                                <div class="title">
                                                    <h4>{{ $AllTimepost->Title }}</h4>
                                                </div>
                                                <div class="description">
                                                    <p>{!! $AllTimepost->Content  !!}</p>
                                                </div>
                                                <div class="details">
                                                    <a href="#" class="btn btn-primary" style="font-size:10px;"><i class="fa fa-comment margin-r-5"></i> {{ $comments[$AllTimepost->Id] }} </a>
                                                    <a href="#" class="btn btn-default" style="font-size:10px;"><i class="fa fa-heart margin-r-5" style="color:red;"></i> {{ $reactions[$AllTimepost->Id] }} </a>

                                                    <a href="/post/{{ $AllTimepost->Id }}" class="btn btn-success" style="font-size:10px;">شاهد المقال </a>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
